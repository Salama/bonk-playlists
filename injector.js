			...
			if(j0V[23].map.m.dbv === 2) {
				...
				if(playlists[t7V[7][1]].b1maps.map(e => {return e.id}).includes(j0V[23].map.m.dbid)) {
					...
					if(j0V[23].map.m.date === "" || j0V[23].map.m.vu === undefined || j0V[23].map.m.vd === undefined) {
						d8I("* Map could not be added to the playlist! This could be because the map is from Bonk 1 and it was automatically selected by the game when the lobby was created.", "#cc3333", true);
					}
					else {
			...
			if(list.maps.length > 0 && list.maps !== "new") {
				...
				$[w3p[6][656]](S9L.W1E(3382), {
					...
					if (d3p[0][0][d3p[5][61]] != S9L.W1E(1936)) {
						...
						filteredMapList.maps = filteredMapList.maps.concat(list.b1maps);
